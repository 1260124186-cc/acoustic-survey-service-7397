...
func TestCloseRejectsSurveyWithOnlyUnusableReadings(t *testing.T) {
	...
	if response := request(http.MethodPost, "/v1/surveys", `{"id":"quality-close","area":"bay","band":"coastal-38khz"}`); response.Code != http.StatusCreated {
		t.Fatalf("create status = %d", response.Code)
	}
	if response := request(http.MethodPost, "/v1/surveys/quality-close/activate", ""); response.Code != http.StatusOK {
		t.Fatalf("activate status = %d", response.Code)
	}
	if response := request(http.MethodPost, "/v1/surveys/quality-close/readings", `{"frequency_hz":42000,"echo_db":-48,"noise_db":-70,"depth_m":30}`); response.Code != http.StatusCreated {
		t.Fatalf("reading status = %d", response.Code)
	}
	if response := request(http.MethodPost, "/v1/surveys/quality-close/close", ""); response.Code != http.StatusBadRequest {
